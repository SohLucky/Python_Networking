...
def handle_client_recv(sock, addr):
	"""Receive messages from client and broadcast them to other clients
...
def handle_disconnect(sock, addr):
	...
	# If we find a queue then this disconnect has not et been handled
	if q:
		q.put(None)
		del send_queues[fd]
...
if __name__ == "__main__":
	...
	while True:
		...
		recv_thread.start()
		send_thread.start()
		print("Conncetion from {}".format(addr))
